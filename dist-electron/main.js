...
class PlaywrightBrowser {
  ...
  async initialize() {
    ...
    if (!fs$1.existsSync(this.downloadPath)) {
      fs$1.mkdirSync(this.downloadPath, { recursive: true });
    }
    ...
    this.page.on("download", async (download) => {
      const downloadPath = path$1.join(this.downloadPath, download.suggestedFilename());
  ...
  async screenshot(options) {
    if (!this.page) throw new Error("Page not initialized");
    const screenshotPath = options?.path || path$1.join(this.downloadPath, `screenshot-${Date.now()}.png`);
  ...
  async waitForSelector(selector, timeout) {
    if (!this.page) throw new Error("Page not initialized");
    await this.page.waitForSelector(selector, { timeout: timeout || 1e4 });
  }
  async waitForNavigation(timeout) {
    if (!this.page) throw new Error("Page not initialized");
    await this.page.waitForLoadState("networkidle", { timeout: timeout || 3e4 });
  }
  async getUrl() {
    if (!this.page) throw new Error("Page not initialized");
...
class MemoryBank {
  ...
  async add(type2, content, metadata2 = {}) {
  ...
  async getRecent(limit2 = 10) {
    ...
    try {
      ...
      const items2 = results.ids.map((id2, index) => ({
        id: id2,
  ...
  async getStats() {
    ...
    try {
      ...
      for (const metadata2 of results.metadatas) {
        if (metadata2?.type === "conversation") conversations++;
        if (metadata2?.type === "search") searches++;
      }
      return {
...
const store = new ElectronStore({
  defaults: {
    downloadFolder: path$1.join(app$1.getPath("downloads"), "comet-search"),
    memoryBankFolder: path$1.join(app$1.getPath("userData"), "memory-bank"),
    memoryEnabled: true,
    memoryMaxItems: 1e3,
    braveApiKey: "",
    kimiApiKey: "",
...
ipcMain$1.handle("browser:navigate", async (_, url) => {
...
ipcMain$1.handle("browser:screenshot", async (_, options) => {
  ...
  return await browser.screenshot({
    ...options,
    path: options?.path ? path$1.join(downloadFolder, options.path) : void 0
  });
});
...
ipcMain$1.handle("memory:add", async (_, type2, content, metadata2) => {
  if (!memory) throw new Error("Memory not initialized");
  if (!store.get("memoryEnabled")) return null;
  return await memory.add(type2, content, metadata2);
});
...
ipcMain$1.handle("shell:openExternal", async (_, url) => {
